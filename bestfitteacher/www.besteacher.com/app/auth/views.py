...
@auth.route("/teacherprofilepagenoedit<int:teacherid>'", methods=['GET', 'POST'])
def teacherprofilepagenoedit(teacherid):
    # thisuseremail = session['email']
    # thisuseremail = thisuseremail.replace("@", "at")
    # thisuseremail = thisuseremail.replace(".", "dot")
    # profileimagepath = "/".join([thisuseremail, 'profilephoto'])
    ...
    form = TeacherProfileForm()
    teacherid=teacherid
    thisteacher = Teacher.query.filter_by(id=teacherid).first()
    # teachernames = 'atul'
    # if form.validate_on_submit():
    #
    ...
    #     form.Country.data = thisteacher.Country
    #     flash('Profile updated successfully!')
    #     return render_template('auth/teacherprofilepagenoedit.html',thisteacher=thisteacher, form=form)
    #     # , teachernames=teachernames,profileimagefile=profileimagefile)
...
def uploadprofilephoto():
    ...
    for file in request.files.getlist("file"):
        file.filename="profilephoto.jpg"
        # '''uploadedimages/'
        target1 = os.path.join(THIS_DIR, thisuserdirectory)
        target = os.path.join(target1, thisuseremail)
        profileimagepath = "/".join([target, 'profilephoto'])
        ...
        file.save(filepath)
        print(filepath)
    return render_template('auth/teacherprofilepage.html', thisteacher=thisteacher, form=form)
    # , teachernames=teachernames, profileimagefile=profileimagefile)
...
def teacherslist():
    ...
    if session['authenticated']==True:
        try:
            ...
            return  render_template('auth/teachernames.html', allteachers=allteachers)
...
@auth.route("/searchteacher", methods=['GET', 'POST'])
def searchteacher():
    look_for = request.form['querystring']
    look_for = '%{0}%'.format(look_for)
    qualification = Teacher.query.filter(Teacher.Qualifications.ilike(look_for))
    experiences = Teacher.query.filter(Teacher.Experiences.ilike(look_for))
    firstname = Teacher.query.filter(Teacher.FirstName.ilike(look_for))
    lastname = Teacher.query.filter(Teacher.LastName.ilike(look_for))
    query1 = qualification.union(experiences)
    query1 = query1.union(firstname)
    query1 = query1.union(lastname)
    query1=query1.distinct()
    # query1 = qualification.filter(Teacher.id.in_(experiences.subquery()))
    # query2 = lastname.filter(Teacher.id.in_(firstname.subquery()))
    return render_template('auth/teachernames.html', allteachers=query1)
    # return  render_template('courses.html')